...
io.on('connection', (socket) => {
  ...
  socket.on('InviteRoomRequest', (invitedPlayerSocketID) => {
    InviteRoomRequest(socket, invitedPlayerSocketID);
  });
...
function startGame(tournamentGameNumber, tournamentID, tournamentNumber, isLastGame) {
  console.log(" tournament No ", tournamentNumber, " startGame was started no", tournamentGameNumber, " TournamentID ", tournamentID);

  TournamentOrdeyByWonGames(tournamentID);
  MatchTournamentPlayers(tournamentID, isLastGame);
}
...
function MatchTournamentPlayers(tournamentID, isLastGame) {
  ...
  for (i = 0; i < (TournamentRooms.get(tournamentID).length + (TournamentRooms.get(tournamentID).length % 2)); i = i + 2) {
    ...
    addMatch(matchingRoomsID, 0, 3, isLastGame);
  }
}
    ...
    await setTimeout(() => {
      if (TournamentRooms.get(tournament.tournamentID).length != 0) {
        if (tournament.tournamentGames.length == tournamentGameNumber + 1) {
          startGame(tournamentGameNumber, tournament.tournamentID, tournamentNumber, true);
        } else {
          startGame(tournamentGameNumber, tournament.tournamentID, tournamentNumber, false);
        }
...
function InviteRoomRequest(socket, invitedPlayerSocketID) {
  console.log("InviteRoomRequest ", invitedPlayerSocketID);

  if (Players.has(invitedPlayerSocketID) && Players.get(invitedPlayerSocketID).playerRoomID != Players.get(socket.id).playerRoomID) {
